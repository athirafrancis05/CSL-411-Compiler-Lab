...
int idx(char c){ for(int i=0;i<8;i++) if(syms[i]==c) return i; return -1; }
...
void preprocess(char*in,char*out){
    int j=0; 
    for(int i=0;in[i];i++) if(!isspace(in[i])) out[j++]=isalpha(in[i])?'i':in[i];
    out[j++]='$'; out[j]=0;
}
...
void parse(char in[]){
    ...
    while(1){
        ...
        else if(r=='>'){ if(!reduce()){ printf("Error: Invalid reduction\n"); return; } }
